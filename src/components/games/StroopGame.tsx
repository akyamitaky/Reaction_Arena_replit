...
export default function StroopGame({ round, addScore, nextRound }: GameContext) {
  ...
  return (
    <div className="flex flex-col items-center gap-8 w-full max-w-sm">
      ...
      <p className="text-6xl font-black" style={{ color: q.inkColor.hex }}>{q.wordText}</p>
      <div className="grid grid-cols-2 gap-3 w-full">
        {q.options.map(o => (
          <button key={o} onClick={() => handleAnswer(o)} className="p-4 rounded-xl border bg-card font-bold hover:border-primary transition-all">
            {o}
          </button>
        ))}
      </div>
