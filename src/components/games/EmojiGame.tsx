...
const SETS = [
  { normal: '😀', odd: '😃' }, { normal: '🍎', odd: '🍏' }, { normal: '🐶', odd: '🐕' },
  { normal: '❤️', odd: '💗' }, { normal: '⭐', odd: '🌟' }, { normal: '🔵', odd: '🔷' },
  { normal: '🌸', odd: '🌺' }, { normal: '🐱', odd: '😺' }, { normal: '🎵', odd: '🎶' },
  { normal: '👋', odd: '🤚' }, { normal: '🏀', odd: '🟠' }, { normal: '🍕', odd: '🍖' },
];
...
export default function EmojiGame({ round, addScore, nextRound }: GameContext) {
  ...
  return (
    <div className="flex flex-col items-center gap-6">
      ...
      <div className="grid grid-cols-3 gap-3">
        {q.items.map((emoji, i) => (
          <button key={i} onClick={() => handleTap(i)} className="w-20 h-20 rounded-xl border bg-card flex items-center justify-center text-4xl hover:scale-110 transition-transform">
            {emoji}
          </button>
        ))}
      </div>
