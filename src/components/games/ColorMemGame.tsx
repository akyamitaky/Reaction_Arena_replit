...
export default function ColorMemGame({ round, addScore, nextRound }: GameContext) {
  ...
  const showSequence = useCallback((seq: typeof COLORS[0][]) => {
    ...
    setTimeout(() => { setPhase('pick'); setActiveIdx(-1); }, seq.length * 700 + 200);
  }, []);
  ...
  return (
    <div className="flex flex-col items-center gap-6">
      {phase === 'show' && (
        <>
          ...
          <div className="flex gap-3">
            {sequence.map((c, i) => (
              <div key={i} className={`w-14 h-14 rounded-xl transition-all ${activeIdx === i ? 'scale-110' : 'scale-75 opacity-30'}`} style={{ backgroundColor: c.hex }} />
            ))}
          </div>
        </>
      )}
      {(phase === 'pick' || phase === 'done') && (
        <>
          <p className="text-sm text-muted-foreground">{phase === 'pick' ? `Pick color ${playerInput.length + 1} of ${sequence.length}` : feedback}</p>
          <div className="grid grid-cols-3 gap-3">
            {COLORS.map(c => (
              <button key={c.name} onClick={() => handlePick(c.name)} className="w-16 h-16 rounded-xl hover:scale-110 transition-transform" style={{ backgroundColor: c.hex }} />
            ))}
          </div>
        </>
      )}
