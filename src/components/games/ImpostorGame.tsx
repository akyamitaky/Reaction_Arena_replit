...
export default function ImpostorGame({ round, addScore, nextRound }: GameContext) {
  ...
  return (
    <div className="flex flex-col items-center gap-6">
      ...
      <div className="grid grid-cols-4 gap-2">
        {q.items.map((ch, i) => (
          <button key={i} onClick={() => handleTap(i)} className="w-16 h-16 rounded-xl border bg-card flex items-center justify-center text-3xl font-mono hover:scale-110 transition-transform">
            {ch}
          </button>
        ))}
      </div>
