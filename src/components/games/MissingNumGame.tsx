...
export default function MissingNumGame({ round, addScore, reportWrong, nextRound }: GameContext) {
    ...
    <div className="flex flex-col items-center gap-6 w-full max-w-sm">
      ...
      <div className="flex gap-2">
        {q.display.map((n, i) => (
          <span
            key={i}
            className={`w-12 h-12 flex items-center justify-center rounded-xl font-black text-lg ${n === null ? 'border-2 border-dashed border-primary text-primary' : 'bg-secondary'}`}
          >
            {n ?? '?'}
          </span>
        ))}
      ...
      <div className="grid grid-cols-4 gap-2 w-full">
        {q.options.map(o => (
          <button
            ...
            className="p-3 rounded-xl border bg-card font-bold text-lg hover:border-primary transition-all"
          >
            {o}
          </button>
        ))}
